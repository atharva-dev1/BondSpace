...
export default function AlbumDetailClient() {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setMedia((prev: any[]) => [data.media, ...prev]);
            setUploading(false);
            setUploadProgress(0);
            if (fileInputRef.current) fileInputRef.current.value = '';
                            ...
                            >
                                ...
                                {item.media_type === 'video' && (
                                    <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-md p-1 rounded">
                                        <PlayIcon size={10} fill="white" className="ml-0.5" />
                                    </div>
                                )}
                            </motion.div>
                        ))}
...
function PlayIcon({ size, fill, className }: { size: number, fill: string, className?: string }) {
    return (
